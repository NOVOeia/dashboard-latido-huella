...
interface Attendee { id:string;registration_id:string;attendee_index:number;is_primary:boolean;is_minor:boolean;full_name:string;email:string|null;phone:string|null;document_id:string|null;amount_cents:number;birthdate:string|null;photo_url:string|null;created_at:string;email1_sent_at:string|null }
                        ...
                        adults.filter(a=>!a.is_primary).forEach(a=>{
                          rows.push(
                            <tr key={a.id} style={{borderTop:'1px solid rgba(255,255,255,0.04)',background:'rgba(255,255,255,0.015)'}}>
                              ...
                              <td className="px-4 py-2"><Btn icon="📝" color="bg-blue-500/15 text-blue-400" onClick={()=>setContractM({name:r.full_name,email:a.email||r.email,recordId:r.id,table:'registrations_5k',contractToken:r.contract_token,contractSignedAt:r.contract_signed_at,contractPdfUrl:r.contract_pdf_url,email1SentAt:a.email1_sent_at||r.email1_sent_at,email2SentAt:r.email2_sent_at})}/></td>
