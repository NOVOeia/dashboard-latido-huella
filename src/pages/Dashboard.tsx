...
interface Attendee { id:string;registration_id:string;attendee_index:number;is_primary:boolean;is_minor:boolean;full_name:string;email:string|null;phone:string|null;document_id:string|null;amount_cents:number;birthdate:string|null;photo_url:string|null;created_at:string;email1_sent_at:string|null;contract_token:string|null;contract_signed_at:string|null;contract_pdf_url:string|null }
...
function ContractModal({name,email,recordId,table,contractToken,contractSignedAt,contractPdfUrl,email1SentAt,email2SentAt,onClose}:{
  ...
  const sendEmail=async(tipo:'email1'|'email2'|'recordatorio')=>{
    ...
    try{
      let subject='',html=''
      if(tipo==='email1'){
        subject='Tu registro en Latido y Huella 2026 esta confirmado'
        const buildHtml=(nombre:string,firmaUrl:string)=>`<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto"><div style="background:#0D1B6E;padding:32px;text-align:center;border-radius:16px 16px 0 0"><img src="https://assets.cdn.filesafe.space/fSGKFAskjzH7pBxfOSIj/media/6a0b45bdc474827cc4087698.png" style="height:60px" alt="Latido y Huella"/><p style="color:rgba(255,255,255,0.7);font-size:13px;margin:12px 0 0">26 de julio de 2026 - Parque COMFAMA Llanogrande</p></div><div style="background:white;padding:32px;border-radius:0 0 16px 16px"><h1 style="color:#0D1B6E;font-size:24px;margin:0 0 8px">Hola ${nombre}!</h1><p style="color:#444;font-size:15px;line-height:1.7">Tu registro para la <strong>Caminata Canina 6.5K Pet Lovers</strong> en Latido y Huella 2026 esta confirmado.</p><div style="background:#f0f4ff;border-radius:14px;padding:20px;margin:16px 0"><p style="margin:0 0 8px;color:#333"><strong>Fecha:</strong> Domingo 26 de julio de 2026</p><p style="margin:0 0 8px;color:#333"><strong>Hora:</strong> 7:00 AM</p><p style="margin:0;color:#333"><strong>Lugar:</strong> Parque del Bienestar COMFAMA Llanogrande</p></div><div style="background:#fff8e1;border-left:4px solid #FFB300;padding:16px;margin:16px 0"><p style="color:#e65100;font-weight:700;margin:0 0 8px">Paso importante - Firma tu consentimiento</p><p style="color:#555;font-size:14px;margin:0 0 12px">Para completar tu registro debes firmar el consentimiento informado.</p><div style="text-align:center"><a href="${firmaUrl}" style="background:#00BCD4;color:white;padding:12px 28px;border-radius:10px;text-decoration:none;font-weight:bold;display:inline-block">Firmar consentimiento</a></div></div><div style="background:#f0fdf4;border-radius:12px;padding:16px;margin:16px 0;border:1px solid #c3f0ca"><h3 style="color:#1b5e20;font-size:15px;margin:0 0 10px">Recomendaciones para las mascotas</h3><p style="color:#2e7d32;font-size:13px;margin:0 0 6px">- Cuídala con collar, placa y trailla.</p><p style="color:#2e7d32;font-size:13px;margin:0 0 6px">- Mucha agua para ti y tu mascota.</p><p style="color:#2e7d32;font-size:13px;margin:0 0 6px">- Desparasítala antes de ir.</p><p style="color:#2e7d32;font-size:13px;margin:0">- Porta el carné de vacunas.</p></div><div style="background:#f3e5f5;border-radius:12px;padding:16px;margin:16px 0"><h3 style="color:#4a148c;font-size:15px;margin:0 0 10px">Entrega de kits</h3><p style="color:#555;font-size:13px;margin:0 0 8px">Tendremos dos lugares de entrega. En el proximo email podras seleccionar el tuyo.</p><p style="color:#6a1b9a;font-size:13px;font-weight:700;margin:0 0 4px">Medellin - Vitrina Chery, Calle 31 # 43-73</p><p style="color:#6a1b9a;font-size:13px;font-weight:700;margin:0">Llanogrande - La Finca de Rigo, Glorieta El Tablazo</p></div><p style="color:#888;font-size:12px;text-align:center;margin:16px 0 0">eventos@latidoyhuella.co - WhatsApp +57 333 277 7912</p></div></div>`
        html=buildHtml(name,contractUrl)
        // Enviar a acompanantes tambien
        if(table==='registrations_5k'){
          const {data:att}=await supabase.from('registration_attendees').select('full_name,email').eq('registration_id',recordId).eq('is_primary',false).not('email','is',null)
          if(att&&att.length>0){
            for(const a of att){
              if(a.email){
                await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-email`,{
                  ...
                  body:JSON.stringify({to:a.email,subject,html:buildHtml(a.full_name||name,contractUrl),from:'eventos@latidoyhuella.co',type:'5k'})
                })
              }
            }
          }
      ...
      const res=await fetch('https://api.resend.com/emails',{
        method:'POST',
        headers:{'Authorization':`Bearer ${import.meta.env.VITE_RESEND_KEY}`,'Content-Type':'application/json'},
        body:JSON.stringify({from:'Latido y Huella <eventos@latidoyhuella.co>',to:[email],subject,html})
      })
      if(res.ok){
                        ...
                        adults.filter(a=>!a.is_primary).forEach(a=>{
                          rows.push(
                            <tr key={a.id} style={{borderTop:'1px solid rgba(255,255,255,0.04)',background:'rgba(255,255,255,0.015)'}}>
                              ...
                              <td className="px-4 py-2">{a.email1_sent_at?<span className="text-xs font-bold text-emerald-400">✉️ {fmtDate(a.email1_sent_at)}</span>:<span className="text-xs text-amber-400">⏳ Sin email</span>}</td>
                              <td className="px-4 py-2"><Btn icon="📝" color="bg-blue-500/15 text-blue-400" onClick={()=>setContractM({name:a.full_name,email:a.email||r.email,recordId:a.id,table:'registration_attendees',contractToken:a.contract_token,contractSignedAt:a.contract_signed_at,contractPdfUrl:a.contract_pdf_url,email1SentAt:a.email1_sent_at,email2SentAt:undefined})}/></td>
                        ...
                        rows.push(
                          <TR key={team.id}>
                            ...
                            <TD><div className="flex gap-1">
                              ...
                              <Btn icon="➕" color="bg-emerald-500/15 text-emerald-400 hover:bg-emerald-500/25" onClick={()=>setCreateM({table:'sports_team_players',fields:[{key:'name',label:'Nombre completo',required:true},{key:'cedula',label:'Cédula'},{key:'ti',label:'TI (menores)'},{key:'age',label:'Edad',type:'number'},{key:'email',label:'Email'},{key:'phone',label:'Teléfono'},{key:'responsable_name',label:'Nombre responsable'},{key:'responsable_phone',label:'Tel. responsable'}],title:`Agregar jugador — ${team.team_name||team.captain_name}`,defaults:{team_id:team.id,is_captain:false,player_index:teamPlayers.length}})}/>
