...
function SignatureCanvas({ onSign }: { onSign: (dataUrl: string) => void }) {
  ...
  useEffect(() => {
    ...
    const end = () => {
      ...
      if (signed) onSign(canvas.toDataURL('image/png'))
    }
  ...
  return (
    <div>
      <div className="relative border-2 rounded-xl overflow-hidden" style={{ borderColor: signed ? CYAN : '#ddd' }}>
      ...
      <button onClick={clear} className="mt-2 text-xs px-3 py-1 rounded-lg border hover:bg-gray-50"
